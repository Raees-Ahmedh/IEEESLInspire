-- ====================================================================
-- ADMIN DASHBOARD ANALYTICS QUERIES FOR EDUCATIONAL PLATFORM
-- Database: PostgreSQL
-- Schema: IEEESLInspire Educational Platform
-- ====================================================================

-- ====================================================================
-- 1. DATABASE COMPLETION STATUS
-- Objective: Show completeness percentage for Universities and Courses
-- ====================================================================

-- Universities Completion Analysis
-- Universities are considered complete if they have imageUrl, contactInfo, and logoUrl
...
-- Courses Completion Analysis
-- Courses are considered complete if they have name, description, and requirement_id
...
-- Objective: Display task distribution across different statuses
-- ====================================================================
...
WITH task_status_counts AS (
  ...
  WHERE audit_info->>'created_at' >= (CURRENT_DATE - INTERVAL '30 days')::text
    OR audit_info->>'created_at' IS NULL -- Include tasks without specific creation date
  GROUP BY status
),
...
-- ====================================================================
-- 3. TASK ASSIGNMENT & DURATION ANALYTICS
-- Objective: Analyze task workflow efficiency and completion times
-- ====================================================================
...
WITH task_durations AS (
  SELECT 
    ...
    -- Calculate duration based on audit_info creation and completion
    CASE 
    ...
    -- Task creation date from audit_info
...
-- Objective: Track platform growth by user role over time
-- ====================================================================
...
WITH user_creation_data AS (
  SELECT 
    user_id,
    role,
    -- Extract creation date from audit_info JSON
...
-- ====================================================================
-- 5. TOP SEARCH & FILTER USAGE
-- Objective: Identify most used search fields and filter criteria
-- ====================================================================
...
WITH search_criteria_analysis AS (
  SELECT 
    search_id,
    search_criteria,
    -- Extract common filter fields from JSON
...
-- ====================================================================
-- 6. SEARCH PERFORMANCE SUMMARY
-- Objective: Monitor search effectiveness and user experience
-- ====================================================================
...
-- Recent daily trends (separate query for chart data)
SELECT 
...
-- ====================================================================
-- 7. TOP BOOKMARKED COURSES
-- Objective: Identify most popular programs based on user engagement
-- ====================================================================
...
course_details_with_bookmarks AS (
  SELECT 
    ...
    -- Calculate bookmark rate (if course analytics exist)
    CASE 
...
-- Objective: Single query for dashboard overview cards
-- ====================================================================
